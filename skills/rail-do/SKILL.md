...
compatibility: Requires an agent (Claude Code, etc.) and Node.js >= 18 (for cli.ts).
metadata:
  author: custom
  version: "3.0"
---

Execute tasks from a planned task. Uses the agentrail runtime for DAG-based ordering, checkpointing, and verification. Drives the agent's native progress bar via TaskCreate/TaskUpdate.

**Input**: Optionally a task name. If omitted, infer from context or prompt for selection.

**Steps**

0. **Locate runtime** — Find `agentrail/cli.ts` in the agent config directory:
   ```bash
   RT=$(find "$HOME" -maxdepth 4 -type f -path "*/agentrail/cli.ts" 2>/dev/null | head -1)
   ```
   If `$RT` is empty, skip all runtime calls and use markdown-only mode.

1. **Select and validate task** — Announce "Using task: <name>". Verify `agentrail/tasks/<name>/` exists with `tasks.md`.

2. **Read all context files** — proposal.md, design.md, tasks.md, log.md.

3. **Load runtime state** — `npx tsx $RT status <name>`. If `$RT` is empty or `_runnable.done: true`, skip to step 10a.

4. **Check for compact recovery** — `npx tsx $RT next-checkpoint <name>` + status. Recovery logic:
...
5. **Show progress and register tasks for native UI** —
   ...
   Then use **TaskCreate** for each pending task to register them in the agent's native progress bar:
   ```
...
7. **Execute tasks (loop)** —
   ...
   a. `npx tsx $RT next <name>` → if `done: true`, go to step 10a. If deadlock, warn and pause.
   ...
   d. **Write pre-task checkpoint:**
      ```bash
      npx tsx $RT checkpoint <name> "starting-task-<N>" --files="<known-files>" --artifacts="<produced-files>"      ```
   ...
   e. Execute the task. After success:
      - Mark runtime done: `npx tsx $RT step-done <name> <index> --artifacts="<produced-files>"`
   ...
   **Pause if:** task ambiguous → AskUserQuestion; design issue → suggest `/railplan <name>`; error/blocker → report; user interrupts; manual action needed.
...
8. **Run verification** — `npx tsx $RT verify <name>`. Report results.

9. **Session end logging** —

   **a. All complete** — `npx tsx $RT complete <name>`. Append to log.md:
   ```markdown
   ...
   **Verification:** <summary>
   ```
   All native tasks will show ✔. Suggest `/raildone <name>`.
   ...
   **c. Other pauses** — Do NOT auto-write. Prompt: "Run `/raillog <name>` to checkpoint."
...
- Use `npx tsx $RT next` for ordering — never parse tasks.md manually.
